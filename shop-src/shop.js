...
(function () {
  ...
  const Drawer = {
    ...
    render() {
      ...
      this.el.innerHTML = '<div class="head"><h2>Your cart <span class="muted small">(' + Store.count() + ')</span></h2><button type="button" class="sh-icon-btn" data-close aria-label="Close">' + icon('x') + '</button></div>' +
        '<div class="items">' + (lines.length ? lines.map(lineHtml).join('') : UI.empty('shopping-cart', 'Your cart is empty.', 'Add a product and it will appear here.', 'Start shopping', url('/shop/'))) + '</div>' +
  ...
  const Pages = {
    ...
    async product() {
      await Store.init(); const p = Store.product(document.body.dataset.product); if (!p) { $('#pdp').innerHTML = UI.empty('package', 'Product unavailable', 'This product is no longer listed.', 'Back to the shop', url('/shop/')); return; }
  ...
  const Checkout = {
    ...
    start() {
      ...
      if (!Store.cart.length) { $('#checkout').innerHTML = UI.empty('shopping-cart', 'Your cart is empty.', 'Add a product before checking out.', 'Start shopping', url('/shop/')); return; }
      this.render();
    },
    ...
    render() {
      ...
      if (this.step === 1) panel = '<div class="co-panel"><h2>Shopping cart</h2><div>' + Store.cart.map(lineHtml).join('') + '</div><div class="co-actions"><a class="sh-btn sh-btn-ghost" href="' + url('/shop/') + '">' + icon('arrow-left', 'sm') + 'Continue shopping</a><button type="button" class="sh-btn sh-btn-primary" data-next>Continue' + icon('arrow-right', 'sm') + '</button></div></div>';

...
(function () {
  ...
  function Picker(p, root, opts) {
    ...
    function render() {
      ...
      if (p.options.shade) parts.push('<div class="opt-group"><div class="lbl">' + (p.options.shade2 ? 'Bottle 1 Shade' : 'Shade') + ' <span>' + esc(state.shade) + '</span></div><div class="opts">' + p.options.shade.map((s) => { const n = Store.stock(p, state.size, s); return '<button type="button" class="opt' + (s === state.shade ? ' on' : '') + (n > 0 ? '' : ' off') + '" data-shade="' + esc(s) + '"><i class="dot" style="background:' + (colors[s] || '#ccc') + '"></i>' + esc(s) + '</button>'; }).join('') + '</div></div>');
      if (p.options.shade2) parts.push('<div class="opt-group"><div class="lbl">Bottle 2 Shade <span>' + esc(state.shade2) + '</span></div><div class="opts">' + p.options.shade2.map((s) => '<button type="button" class="opt' + (s === state.shade2 ? ' on' : '') + '" data-shade2="' + esc(s) + '"><i class="dot" style="background:' + (colors[s] || '#ccc') + '"></i>' + esc(s) + '</button>').join('') + '</div></div>');

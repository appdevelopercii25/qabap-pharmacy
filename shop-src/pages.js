...
function shopGrid(base, catalog, category) {
  ...
  return '<main id="shop-main" class="shop"><div class="wrap shop-page">' + crumbs(base, cat ? [['Shop', base + '/shop/'], ['Toppik', base + '/shop/toppik/'], [cat.name]] : [['Shop', base + '/shop/'], ['Toppik']]) +
    (cat ? '' : '<div class="shop-banner shop-slides" aria-label="Toppik">' + ['slide-1-toppik.jpg', 'slide-2-award.jpg', 'slide-3-man-side.jpg', 'slide-4-woman-short.jpg', 'slide-5-woman-long.jpg', 'slide-6-man-crown.jpg'].map((f, n) => '<img src="' + base + '/assets/toppik/' + f + '" alt="' + (n === 0 ? 'Toppik, everything you need to transform fine, thin and thinning hair' : n === 1 ? 'Toppik award winning hair building fibers' : 'Before and after Toppik') + '" width="1916" height="821"' + (n === 0 ? ' class="on"' : ' loading="lazy"') + '>').join('') + '<div class="dots">' + [0, 1, 2, 3, 4, 5].map((n) => '<button type="button" data-slide="' + n + '"' + (n === 0 ? ' class="on"' : '') + ' aria-label="Slide ' + (n + 1) + '"></button>').join('') + '</div></div>') +

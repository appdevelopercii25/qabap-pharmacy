...
function shopGrid(base, catalog, category) {
  ...
  return '<main id="shop-main" class="shop"><div class="wrap shop-page">' + crumbs(base, cat ? [['Shop', base + '/shop/'], ['Toppik', base + '/shop/toppik/'], [cat.name]] : [['Shop', base + '/shop/'], ['Toppik']]) +
    (cat ? '' : '<div class="shop-banner shop-slides" aria-label="Toppik">' + ['slide-1-toppik-v3.jpg', 'slide-2-award-v3.jpg', 'slide-3-man-side-v3.jpg', 'slide-4-woman-long-v3.jpg', 'slide-5-man-crown-v3.jpg'].map((f, n) => '<img src="' + base + '/assets/toppik/' + f + '" alt="' + (n === 0 ? 'Toppik, everything you need to transform fine, thin and thinning hair' : n === 1 ? 'Toppik award winning hair building fibers' : 'Before and after Toppik') + '" width="1920" height="698"' + (n === 0 ? ' class="on"' : ' loading="lazy"') + '>').join('') + '<div class="dots">' + [0, 1, 2, 3, 4].map((n) => '<button type="button" data-slide="' + n + '"' + (n === 0 ? ' class="on"' : '') + ' aria-label="Slide ' + (n + 1) + '"></button>').join('') + '</div></div>') +
